...
const getAuthConfig = (
  ...
  if (!API_KEY) {
    throw new Error(
      "TMDB API key is missing. Check VITE_API_KEY in your .env file.",
    );
  }
...
const getWatchProviders = async (
  id: number,
): Promise<Result<IWatchProvidersResponse>> => {
  try {
    const { data } = await Api.get(
      `${BASE_URL}/${id}/watch/providers`,
      getAuthConfig(),
    );
...
export const getById = async (id: number): Promise<Result<IMoviesList>> => {
  try {
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Failed to load the movie.";
    console.error(errorMessage);
